...
template<typename T>
void destroy_user_list(T user_list)    //销毁两个列表
{
    int count = 0;
...
3.socket,bind,listen,epoll_wait,得到io事件event
...
int main(int argc, char *argv[])
{
    ...
    pool<http_conn> threadpool(10, 10000);  //创建线程池对象
    vector<http_conn*> http_user_list(MAX_FD, NULL);    //连接数量取决于描述符的数量
    int user_count = 0;
    ...
    vector<client_data *> client_data_list(MAX_FD, NULL); //创建用户信息列表，其中包含定时器信息
    ...
    while(!stop_server)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            if(socketfd == listenfd)
            {
                ...
                //开始初始化client_data
                //创建定时器
                if(client_data_list[connfd] == NULL)
                {
            ...
            else if(event_list[i].events & (EPOLLHUP | EPOLLRDHUP | EPOLLERR)) 
            {
                ...
                //关闭对应的计时器
                util_timer *timer = client_data_list[socketfd]->timer;
                if(timer)
            ...
            else if (event_list[i].events & EPOLLIN)
            {
                util_timer * timer = client_data_list[socketfd]->timer;
                //客户的数据要读进来
                if (http_user_list[socketfd]->read_once())
                {
                    //如果读到了一些数据（已经读到了读缓存区里）,就把这个请求放到工作线程的请求队列中
                    threadpool.append(http_user_list[socketfd]);
